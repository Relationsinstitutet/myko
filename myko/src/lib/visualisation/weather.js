...
export default class Drop {
  constructor(weatherType, weatherPos, size, width, p5) {
    this.p5 = p5;
    ...
    this.width = width;
    this.w = this.p5.map(this.startPos.z, 0, 30, 0.4 * this.width, 1.3 * this.width);
    this.h = this.p5.map(this.startPos.z, 0, 30, 4 * this.width, 10 * this.width);
    this.acc = 1;
  ...
  update(windForce) {
    this.wind = windForce;
    this.vel.y = this.p5.map(this.pos.z, 0, 3, 0.5 * this.acc, 1.5 * this.acc);
